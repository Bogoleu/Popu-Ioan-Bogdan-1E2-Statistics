...
#Exercitiul  B1
#  verificam daca un punct se afla in interiorul torului
is_inside_torus <- function(x1, x2, x3, R, r) {
  ((x3^2 + (sqrt(x1^2 + x2^2) - R)^2) <= r^2)
}

# estimam volumul torului folosind metoda Monte Carlo
monte_carlo_torus_volume <- function(R, r, samples) {
  # contorul pentru punctele din interiorul torului , initializare
